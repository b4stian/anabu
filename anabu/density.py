...
class Evaluator:
    ...
    def create_brightness_arrays(self) -> None:
        ...
        interface.logging.info(f"The nonmasked area contains {self.number_total_pixels} pixels which will be evaluated.")
        self.brightness_array = np.linspace(0, 255, 256).astype(int)
        self.brightness_counts = np.array([np.count_nonzero((self.grey_photo == i) * self.cropped_mask) for i in self.brightness_array])
        ...
        self.brightness_cumulative = np.array([np.count_nonzero((self.grey_photo < i+1) * self.cropped_mask) for i in self.brightness_array])
    ...
    def calculate_distribution_params(self) -> None:
        ...
        interface.logging.info(f"The minimum brightness value is {self.brightness_min}.")
        interface.results.add_result(
        ...
        interface.logging.info(f"The maximum brightness value is {self.brightness_max}.")
        interface.results.add_result(
            variable="brightness_max",
            parameter="maximum brightness value",
            value=self.brightness_max,
        )                
        self.brightness_peak = int(np.where(self.brightness_counts == np.amax(self.brightness_counts))[0])
        #FIXME Implement better solution for possibility that two values have the same brightness.
        interface.logging.info(f"The most often found brightness value is {self.brightness_peak}.")
        interface.results.add_result(
        ...
        interface.logging.info(f"The average brightness value is {round(self.brightness_mean,2)}.")
        interface.results.add_result(
        ...
        interface.logging.info(f"The median brightness value is {self.brightness_median}.")
        interface.results.add_result(
        ...
        interface.logging.info(f"The variance of the brightness distribution is {round(self.brightness_variance,2)}.")
        interface.results.add_result(
        ...
        interface.logging.info(f"The standard deviation of the brightness distribution is {round(self.brightness_stdev,2)}.")
        interface.results.add_result(
        ...
        interface.logging.info(f"The skew of the brightness distribution is {round(self.brightness_skew,2)}.")
        interface.results.add_result(
            variable="brightness_skew",
            parameter="skew of brightness distribution",
            value=self.brightness_skew,
        )
        self.brightness_kurtosis = scipy.stats.kurtosis(self.grey_photo[self.cropped_mask])
        interface.logging.info(f"The kurtosis of the brightness distribution is {round(self.brightness_kurtosis,2)}.")
        interface.results.add_result(
        ...
        interface.logging.info(f"The brightness-weighted mean of the brightness distribution is {round(self.brightness_weighted_mean,2)}.")
        interface.results.add_result(
        ...
        interface.logging.info(f"The brightness-squared-weighted mean of the brightness distribution is {round(self.brightness_2_weighted_mean,2)}.")
        interface.results.add_result(
        ...
        interface.logging.info(f"The diversity of the brightness distribution is {round(self.brightness_diversity,2)}.")
        interface.results.add_result(
